...
public class RDFCreate {
	public static void RDFCreater(ArrayList<String> title,ArrayList<String> time,ArrayList<String> article,ArrayList<String> publisher, ArrayList<String> url) throws IOException{
		int j = 0;
		...
		for(int i = 0; i < title.size(); i++){
			...
			Property URL = model.createProperty(SCHEMA_NS + "url");
			model.add(news, URL, url.get(i));
			...
			for(Entry<String,Double> Tag_Forder : entries){
				if(j == 10)
					break;
				Property keywords = model.createProperty(SCHEMA_NS + "keyword");
